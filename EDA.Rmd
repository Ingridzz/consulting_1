...
```{r}
z <- 1.96 #5% alpha error
z_b <- 1.2816  #90% power of the study


#sample size fitness
#select un-duplucated records
ind <- df[!duplicated(df$subj),]
se <- sd(c(ind$PreVo2max, ind$PostVo2max))/sqrt(30)
sd <- sd(c(ind$PreVo2max, ind$PostVo2max))
#standard deviation
delta <- 0.15 #effect size
