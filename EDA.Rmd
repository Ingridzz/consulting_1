...
Figure 2 shows the individual's changes in brain region activation before and after treatment and color coded by treatment group. The light background lines represent individual measurements, and the bold line is a linear regression for each group. There is a general trend of increase in region activation, although the standard errors for the regression are large.
...
\newpage
Figure 4, similarly to Figure 2 illustrates individual's changes before and after treatment with a linear regression line shown in a black dashed line. In most cases individuals had a slight increase in fitness.
...
ggplot(data = df_long, aes(x= time, y = vo2max, group = subj, color = Age))+ 
  ...
  geom_smooth(method = 'lm', show.legend = TRUE,aes(group = Group), size = 1, se = FALSE, color = "black",linetype = "dashed")+
